...
library(igraph)
```
This vignette shows how to cluster the vertices of a weighted graph using the random walk method developed by Horel and Koren [1] with the `Rwclust` package using an example. 

# Introduction

The random walk algorithm of Horel and Koren [1] for clustering weighted graphs is based on the concepts of *connected components* and *cut edges*. Denote a graph $G=(V, E)$ where $V$ is the set of vertices and $E$ is the set of edges. A *connected component* is a subset of vertices that are mutually reachable. A graph can have one connected component constituting the entire graph, or several connected components that for purposes represent clusters. 

A set of *cut edges* is a set $E' \subset E$ such that the number of connected components in $G-E$ is greater than that in $G$. Essentially, $E'$ contains a set of edges whose removal results in the creation of separate clusters of vertices. 

The random walk algorithm finds a set of cut edges but "sharpening" the difference between the weights of edges which connect vertices in a connected component, whose weights grow each iteration of the algorithm, and the weights of edges that run between separate components, which become much smaller. All edges below whose weight lies below a certain user-defined threshold are deleted and the resulting connected components become the clusters.
...
3. delete edges below the cutoff
4. compute the connected components of the resulting graph

# Usage Example

We will use an example graph taken from Horel and Koren. 
...
The data is contained in a dataframe. We use `igraph` to create a graph object a display it along with the edge weights. 
```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=7, fig.height=5}
...
[1] Harel, David, and Yehuda Koren. "On clustering using random walks." *International Conference on Foundations of Software Technology and Theoretical Computer Science*. Springer, Berlin, Heidelberg, 2001.
